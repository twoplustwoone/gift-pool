...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    ...
    if (intent === 'reorder-items') {
      const sourceCategoryId = normalizeCategoryId(
        formData.get('sourceCategoryId'),
      );
      const targetCategoryId = normalizeCategoryId(
      ...
      if (sameCategory) {
        ...
        const movedOutIds = sourceActualIds.filter(
          (id) => !sourceRemaining.has(id),
        );
        invariantResponse(
      ...
      const updateItems = sameCategory
        ...
        : [
            ...buildDenseSortOrder(sourceOrderedItemIds).map(
              ({ id, sortOrder }) =>
                prisma.wishlistItem.update({
                  where: {
                    id,
            ...
            ...buildDenseSortOrder(targetOrderedItemIds).map(
              ({ id, sortOrder }) =>
                prisma.wishlistItem.update({
                  where: {
                    id,
