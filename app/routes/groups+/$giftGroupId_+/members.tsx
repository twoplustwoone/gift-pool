...
const GroupMembersRoute = () => {
  ...
  return (
    <Card padding="lg">
      <div className="mb-1 text-lg font-semibold">Group Members ({totalMembers})</div>
            ...
            <li key={m.user.id} className="flex items-center gap-3 p-3">
              <Link to={`/users/${m.user.username}`} className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">
                    {birthday ? `Birthday ${birthday.toLocaleDateString()}` : 'Birthday not set'}
                  </div>
                </div>
              </Link>
              ...
              <div className="ml-auto flex items-center gap-3">
                <div className="text-right">
                  <div className="text-sm font-semibold">${(giftBudget / 100).toFixed(2)}</div>
                ...
                {canOwner && !isViewer && (
                  <div className="flex items-center gap-1">
                    {m.role === 'MEMBER' ? (
                      <fetcher.Form method="post" action={`/groups/${giftGroup.id}/settings`}>
                        <input type="hidden" name="giftGroupId" value={giftGroup.id} />
                        <input type="hidden" name="memberUserId" value={m.user.id} />
                        <Button size="icon" variant="secondary" name="intent" value="member-promote-admin" aria-label="Promote to admin">
                          <Icon name="plus" />
                        </Button>
                      </fetcher.Form>
                    ) : (
                      <>
                        <fetcher.Form method="post" action={`/groups/${giftGroup.id}/settings`}>
                          <input type="hidden" name="giftGroupId" value={giftGroup.id} />
                          <input type="hidden" name="memberUserId" value={m.user.id} />
                          <Button size="icon" variant="secondary" name="intent" value="member-demote-member" aria-label="Demote to member">
                            <Icon name="reset" />
                          </Button>
                        </fetcher.Form>
                        <fetcher.Form method="post" action={`/groups/${giftGroup.id}/settings`}>
                          <input type="hidden" name="giftGroupId" value={giftGroup.id} />
                          <input type="hidden" name="newOwnerUserId" value={m.user.id} />
                          <Button size="icon" variant="secondary" name="intent" value="ownership-transfer" aria-label="Transfer ownership">
                            <Icon name="person" />
                          </Button>
                        </fetcher.Form>
                      </>
                    ...
                    <fetcher.Form method="post" action={`/groups/${giftGroup.id}/settings`}>
                      <input type="hidden" name="giftGroupId" value={giftGroup.id} />
                      <input type="hidden" name="memberUserId" value={m.user.id} />
                      <Button size="icon" variant="destructive" name="intent" value="member-remove" aria-label="Remove member">
                        <Icon name="trash" />
                      </Button>
                    </fetcher.Form>
                  </div>
