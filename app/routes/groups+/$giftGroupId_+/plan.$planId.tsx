...
export default function GiftPlanRoute() {
  ...
  const snapshot = plan.budgetSnapshot
    ? (JSON.parse(plan.budgetSnapshot) as Array<{
        userId: string;
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-4">
        <div className="mb-4 flex items-center gap-3">
          ...
          <div className="text-sm text-muted-foreground">
            Birthday {new Date(plan.birthdayDate).toLocaleDateString()}
          </div>
          <div
            className={`ml-auto rounded px-2 py-0.5 text-xs ${locked ? 'bg-amber-200 text-amber-900' : 'bg-emerald-200 text-emerald-900'}`}
          >
            {plan.status}
          </div>
        </div>
        {locked ? (
          <div className="space-y-2">
            <div className="text-sm">
              Locked by {plan.lockedBy?.username} at{' '}
              {new Date(plan.lockedAt!).toLocaleString()}
            </div>
            <div>
              ...
              <ul className="text-sm">
                {snapshot?.map((s) => (
                  <li key={s.userId}>
                    User {s.userId}: ${(s.contributionCents / 100).toFixed(2)}
                  </li>
                ))}
              </ul>
            </div>
          ...
          <div className="space-y-2">
            <div className="text-sm">
              Plan is editable. Locking will snapshot current budgets.
            </div>
            <Form method="post">
